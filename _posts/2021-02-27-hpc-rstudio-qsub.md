...
The solution is to wrap the PBS commands in an `ssh` command. That way, hhen we want to execute `qsub` from within the container the command is actually ssh'd out of the container. This code is already in the [RStudio container recipe file](https://github.com/Wytamma/rstudio-hpc/blob/master/rstudio-hpc.def). When the container is built a fake `qsub` command is created within the container for your use.
